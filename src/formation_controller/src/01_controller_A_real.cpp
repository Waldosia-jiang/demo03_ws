...
int main(int argc, char** argv) {
    ...
    ros::Publisher follower_vel = nh.advertise<geometry_msgs::Twist>(follower_robot_name + "/cmd_vel_mux/input/teleop", 2); 
    ros::Publisher leader_vel_pub = nh.advertise<geometry_msgs::Twist>("robot1/cmd_vel_mux/input/teleop", 2);
    ...
    while (nh.ok()) {
        ...
        try {
            ...
            listener.lookupTransform(leader_robot_name + "/odom", follower_robot_name + "/odom", ros::Time(0), transform);
        }
        ...
        double dt = (current_vel_time - pre_vel_time).toSec();
        pre_vel_time  =  current_vel_time;
        ...
        // 输出限制幅度
        if(v_follower > 1.2 || v_follower < -1.2 )  v_follower  =  0.6 * abs(v_follower) / v_follower;
        ...
        // 输出限制幅度
        if(w_follower > 0.6 || w_follower < -0.6 )  w_follower  =  0.1* abs(w_follower) / w_follower;
